...
export default class {
  ...
  handleShowTickets(e, bills, index) {
    // Si l'index change, réinitialisez tout
    if (this.index !== index) {
      this.showCounter = 0;
      this.counter = 0;
      this.id = undefined;
    ...
    this.showCounter++;
    // Détachez d'abord tous les gestionnaires d'événements existants
    bills.forEach((bill) => {
      $(`#open-bill${bill.id}`).off("click");
    });

    // Ensuite, attachez de nouveaux gestionnaires d'événements
    bills.forEach((bill) => {
